...
def Texture_single_values(Cat,ind,chan,Lab,simg):
    df=pd.DataFrame([[]])
...
def Mitochondria_measurement(Lab,simg,viz=False):
    df=pd.DataFrame([[]])
    Mseg = Lab * simg > skimage.filters.threshold_multiotsu(Lab * simg)[-1]
...
def RNA_measurement(Lab,simg,viz=False):
    df=pd.DataFrame([[]])
    Rn = simg * ndi.binary_erosion(Lab, structure=skimage.morphology.disk(1))
